...
| H&M, Decathlon, Adidas, The North Face, Vans, Timberland, New Balance | ⚠️ Manual | Confirmado 403 (Akamai / Cloudflare) incluso con navegador headed — el bypass que funcionó en Mango/Womensecret/Zalando/Desigual no es universal |
| Zara | ⚠️ Manual (por ahora) | El bloqueo de Akamai sí se esquiva con navegador headed (HTTP 200 en vez de 403), pero el listado no trae JSON-LD ni `__NEXT_DATA__` ni enlaces de producto nada más cargar — probablemente necesita más scroll/interacción para que el grid virtualizado pinte los productos |
...
| Pull&Bear, Stradivarius, Oysho, Massimo Dutti, C&A, Superdry, Skechers | ⚠️ Manual | HTTP 200 con navegador headed pero sin JSON-LD/`__NEXT_DATA__` de producto ni tarjetas reconocibles en la carga inicial; no investigado a fondo |
